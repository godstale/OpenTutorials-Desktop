...
### 작업 내용
- **기본 튜터 항상 1개 이상 유지 (추가/삭제/수동 관리 시)**:
  - **에이전트 등록(`createExternalAgent`)**: 에이전트 신규 등록 시, 기존에 등록된 기본 튜터(`is_ai_tutor === true`)가 존재하지 않는 경우 새로 생성하는 에이전트를 자동으로 기본 튜터로 지정해 0개 상태가 되는 것을 방지했습니다.
  - **에이전트 삭제(`deleteExternalAgent`)**: 에이전트 삭제 완료 후 남아있는 에이전트가 존재하되 기본 튜터가 없다면(기존 기본 튜터가 삭제된 경우 등), 남은 에이전트 중 첫 번째 에이전트를 기본 튜터로 자동 지정하도록 하였습니다.
  - **에이전트 수정(`updateExternalAgent`)**: 수동으로 기본 튜터 지정을 해제(`is_ai_tutor === false`로 변경)하려 할 때, 해당 에이전트 외에 다른 기본 튜터가 존재하지 않는다면 에러를 반환하여 최소 1개의 기본 튜터 지정을 강제했습니다.
  - **UI 제어(`Agents.tsx`)**: 유일한 기본 튜터인 경우 에이전트 관리 화면에서 "기본 튜터 해제" 메뉴를 비활성화(`disabled`)하여 UI 단에서도 해제가 불가능하도록 처리해 안전성을 높였습니다.
- **신규 강좌 임포트 시 기본 튜터 자동 할당**:
  - **백엔드 임포트 로직(`src-tauri/src/bundle/mod.rs`)**: "강좌 검색" 화면 등을 통해 새로운 강좌 패키지를 추가(임포트)할 때, 만약 수동으로 지정된 기본 튜터가 DB에 존재한다면 해당 튜터의 `agent_id`를 새로 upsert하는 `course_packages` 레코드의 `agent_id` 필드에 기본값으로 자동 할당합니다. 기존 패키지가 재임포트되는 등 기존 `agent_id`가 이미 지정되어 있는 상태라면 기존의 값을 유지하도록 설계했습니다.
  - 이를 통해 강좌가 새로 등록되었을 때 에이전트 정보가 누락되어 "에이전트 없음" 상태로 표기되는 문제를 해결했습니다.

### 변경된 파일
- `src/lib/api/external-agents.ts`
- `src/pages/Agents.tsx`
- `src-tauri/src/bundle/mod.rs`
